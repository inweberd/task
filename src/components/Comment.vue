<template>
	<from-bottom-dialog :page-id="pageId" :modelValue="modelValue"
		@update:modelValue="(e) => $emit('update:modelValue', e)" @cancel="cancel" :show-heng-gang="false" maskMode="light"
		:height="height" tag="comment" mode="white">
				...
				<div class="items">
					<div class="item" :key="i" v-for="(item, i) in comments">
						...
						<div class="replies" v-if="Number(item.sub_comment_count)">
							...
							<Loading v-if="loadChildren && loadChildrenItemCId === item.comment_id" :type="'small'"
