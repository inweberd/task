<template>
	<div id="Music">
		<div class="header">
			...
			<div class="right">
				...
				<template v-if="data.isFixed">
					<img class="star" v-if="data.isCollect" src="../../assets/img/icon/star-yellow.png"
		...
		<div class="content">
			<Scroll class="Scroll" :fixedHeight="180" @fixed="(e) => (data.isFixed = e)" @pulldown="loadData">
				<div class="desc">
					<div class="cover-wrapper" @click="togglePlay()">
						<img class="cover" :src="_checkImgUrl(data.music.cover)" alt="" />
						<img v-if="!data.isPlay" src="../../assets/img/icon/play-white.png" alt="" class="play" />
						<img v-if="data.isPlay" src="../../assets/img/icon/pause-white.png" alt="" class="play" />
					</div>
					<div class="info">
      ...
      <div class="l-button primary" @click="_no">
		...
		<Share v-model="data.isSharing" mode="music" ref="share" pageId="Music" @showDouyinCode="data.showDouyinCode = true"
			@showShare2WeChatZone="data.shareType = 2" @share2WeChat="data.shareType = 3" @share2QQZone="data.shareType = 4"
			@share2QQ="data.shareType = 5" @share2Webo="data.shareType = 8"
			@ShareToFriend="delayShowDialog(() => (data.shareToFriend = true))" />

		<DouyinCode v-model="data.showDouyinCode" />

		<ConfirmDialog v-model:visible="data.showSharePassword" title="你的口令已复制"
			subtitle="0F.:/ a【风就应该自由要什么归宿】长按复制此条消息，打开抖音搜索，聆听音乐##kwu3VCixHl8##[抖音口令]" :okText="data.okText" cancelText="不分享了"
			@ok="data.shareType = -1" @cancel="data.shareType = -1">
