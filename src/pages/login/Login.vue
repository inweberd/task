	...
	<div class="login">
		...
		<div v-else class="content">
			...
			<van-form @submit="onSubmit">
				<van-field v-model="username" label="" name="用户名" placeholder="用户名"
					:rules="[{ required: true, message: '请填写用户名' }]" />
				<van-divider />
				<van-field v-model="password" type="password" name="密码" label="" placeholder="密码"
					:rules="[{ required: true, message: '请填写密码' }]" />
				<div style="margin: 16px;">
					<van-button round block type="primary" native-type="submit" style='background:#F56D17;border:none'>
						登录
					</van-button>
				</div>
			</van-form>
			...
			<van-image width="100" height="70" :src="zhuce"  @click="go('/signUp')" style="display:inline-block"/>
<!--				<van-button style='flex: 1;' type="primary" @click="go('/signUp')">注册</van-button>-->
				<van-image width="100" height="70" :src="mima"  @click="go('/forget')" style="display:inline-block"/>
...
const username = ref('13333333333');
