			...
			<van-tab title="选择提现">
				...
				<van-action-sheet v-model:show="checked" title="选择提现方式">
					<van-radio-group v-model="pay_card_id" disabled>
						<van-card :desc="item.name" :title="item.card_name" v-for="item in state.select.card"
							@click="onSelect(item)" :thumb="item.mode == 'bank' ? imageSrc : alipay">
							<template #bottom>
								<div>
									卡号:{{ item.card_no }}
								</div>
							...
							<template #footer>
								<van-radio checked-color="#ee0a24" style="position: absolute;right: 30rem;bottom: 20rem;"
									:name="item.id" />
							</template>
						</van-card>
					</van-radio-group>
				...
				<van-button style="margin-top: 100rem;
		margin: 100px auto;
		width: 80vw;
		" round block type="primary" @click="goPay">申请提现</van-button>
			</van-tab>
			<van-tab title="绑定提现">
				<van-tabs v-model:active="active_">
					<van-tab title="银行卡" style="padding: 0px 20px;">
						...
						<van-button type="primary" style="margin-top: 30rem" block @click="save('bank')">保存</van-button>
					...
					<van-tab title="支付宝" style="padding: 0px 20px;">
						...
						<van-button type="primary" style="margin-top: 30rem" block @click="save('ali')">保存</van-button>
