	...
	<div id="Message" ref="app" :class="data.createChatDialog ? 'disable-scroll' : ''">
		<div class="no-search" v-show="!data.searching">
			...
			<Scroll ref="mainScroll">
				<div class="friends">
					<div class="friend" @click="nav('/message/chat')" :key="index" v-for="(item, index) in store.friends.all">
			...
			<from-bottom-dialog page-id="Message" v-model="data.createChatDialog">
				<div class="create-chat-wrapper" v-show="!data.showJoinedChat">
					<Search :isShowRightText="data.isShowRightText" @click="data.isShowRightText = true"
						@notice="data.isShowRightText = false" @clear="data.isShowRightText = false" class="ml2r mr2r"
						placeholder="搜索用户" v-model="data.createChatSearchKey"></Search>
					<template v-if="data.createChatSearchKey">
						<div class="search-result" v-if="data.searchFriends.length">
							<div class="search-result-item" :key="i" v-for="(item, i) in data.searchFriends"
					...
					<template v-else>
						...
						<div class="friend-list">
							...
							<div class="friend-item" :key="i" v-for="(item, i) in store.friends.all"
				...
				<div class="recommend-dialog" v-if="data.isShowRecommend">
					<div class="dialog-content">
						<div class="dialog-header">
							...
							<img @click="data.isShowRecommend = false" src="../../assets/img/icon/components/gray-close-full2.png"
								alt="" />
						</div>
		...
		<div class="searching" v-show="data.searching">
			<Search v-model="data.searchKey" right-text="取消" right-text-color="white" @notice="data.searching = false"
				:isShowRightText="true" />
			<div class="more-chat">
				<template v-if="data.searchKey">
					<div class="sub-title" v-if="searchFriendsAll.length">
						<span>联系人</span>
						<div class="right" v-if="searchFriendsAll.length > 3"
							@click="nav('/message/more-search', { key: data.searchKey })">
							<span>更多联系人</span>
							<dy-back mode="gray" img="back" scale=".6" direction="right" />
						</div>
					</div>
					<People v-for="item in searchFriendsAll.slice(0, 3)" :key="item.id" mode="search" :searchKey="data.searchKey"
						:people="item" />
