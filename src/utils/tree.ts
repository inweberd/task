...
export function create(list = [], parentId = null, idKey = 'id', parentIdKey = 'parentId', withRoot = false) {
  ...
  if (parentId && withRoot) {
    const target: any = list.find((item) => {
 ...
 */
export function find(data: any = [], isFindOne: boolean, fn: any, field = 'children') {
  let result: any = [];
 ...
 * @return {Array} 节点路径数组
 */
export function findPath(data: any, fn: any, field = 'children') {
  const path: any = [];

  function find(array: any, parent?: any) {
...
const _flat = function (tree: any, map: any = {}, idProp = 'id', childrenProp = 'children') {
 ...
 * @return {Array} 节点路径数组
 */
export function treeRevert(tree: any, idProp = 'id', childrenProp = 'children') {
  const map = {};
  _flat(tree, map, idProp, childrenProp);
  return Object.values(map);
}

export function findLabel(id: string, list: Array<any>, config = { label: 'label', code: 'code' }) {
  let info =
    find(list, true, (item: any) => {
      return item[config.code] == id;
    }) || {};
  return info[config.label] || '';
}
