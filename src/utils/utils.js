...
class helper{
    ...
    constructor()
    {
        ...
        this.get         = {
            ...
            css    : {
                var:(name)=>this.getCssVar(name)
            }
        }

        // 链式操作 set 属性
        this.set         = {
            cookie : (name,value,exdays,domain,path)   => this.setCookie(name,value,exdays,domain,path),
        ...
        this.is      = {
            ...
            empty : (...args) => this.isEmpty(...args),
            emptyAll: (...args) => this.emptyAll(...args),
        ...
        this.parse  = {
            url    :  {
                query: url => this.parseUrlQuery(url)
            }
        }
        ...
        this.array = {
            ...
            equal  : (array1,array2) => this.arrayEqual(array1,array2)
        }
        ...
        this.string= {
            omit: (string, length, omission, location) => this.stringOmit(string, length, omission, location)
        }
    ...
    createPaging(item = 1, page = 5, numb = 5)
    {
        ...
        if (numb % 2 == 0) {
            ...
            // if(item > pages-(mean+1)) min = pages-(numb-1);
        }
    ...
    getKeyName(keycode)
    {
        ...
        else if (this.config.log) console.warn('Unknow Key(Key Code:' + keycode + ')')
    ...
    getStorage(namespace, key = true)
    {
        ...
        if (this.isEmpty(namespace)) {
            if (this.config.log) console.warn('请输入localStorage的key值!')
        }
        else {
    ...
    setStorage(namespace, key, value)
    {
        ...
        if (this.isEmpty(namespace)) {
            if (this.config.log) console.warn('请输入需要存储的key名称!')
        }
        else if (this.isEmpty(key)) {
            if (this.config.log) console.warn('键值key不得为空!')
        }
        else {
    ...
    hasStorage(key)
    {
        ...
        if (this.isEmpty(key)) {
            if (this.config.log) console.warn('请给一个localStorage的key值')
        }
    ...
    getSession(namespace, key = true)
    {
        ...
        if (this.isEmpty(namespace)) {
            if (this.config.log) console.warn('请输入需要查询的key!')
        }
        else {
    ...
    hasSession(key)
    {
        ...
        if (this.isEmpty(key)) {
            if (this.config.log) console.warn('请给一个sessionStorage的key值')
        }
    ...
    {
        let result = false
        if (this.isEmpty(classOrId)) {
            if (this.config.log) console.warn('请选择需要设置的DOM元素')
        }
        else if (this.isEmpty(css)) {
            if (this.config.log) console.warn('请设置CSS')
        }
        else {
    ...
    dateToTime(date)
    {
        if (this.isEmpty(date)) {
            if (this.config.log) console.warn("请输入一个日期格式，如：2021-5-20 13:14:00")
        }
        else {
    ...
    performance(storage = false)
    {
        ...
        try {
            ...
            if (this.config.log) console.warn(e)
        }
    ...
    isEmpty(...args)
    {
        ...
        const empty = (data) => {
            ...
            if (Array.isArray(data)) {
                ...
                if (Array.prototype.isPrototypeOf(data) && data.length === 0) ok = true

            } else if (typeof data == 'number') ok = isNaN(data)
            else if (!this.isNull(data)) {

                if (data instanceof Object) {

                    if (JSON.stringify(data) === "{}") ok = true

                } else if ((data + '').replace(/(^\s*)|(\s*$)/g, '').length === 0) ok = true
        ...
        // 默认方法
        let func = (array = []) => array.every(item => item)

        // 遍历参数
        for (let item in args) {
            // 找到是方法的参数
            if (typeof args[item] == 'function') func = args[item]
            else result.push(empty(args[item]))
        }

        // 如果全部为 true 则返回 true 否则返回 false
        return func(result)
    }
    ...
    isIdCard(string = "")
    {
        if (this.isEmpty(string)) {
            if (this.config.log) console.warn("请输入一个身份证号码")
        }
        else {
    ...
    isDomain(domain = null) {
        ...
        const rule = /^(([-\u4E00-\u9FA5a-z0-9]{1,63})\.)+([\u4E00-\u9FA5a-z]{2,63})(:[0-9]{1,5})?$/;

        // 过滤 http(s):// 和 / 并 校验规则
        result = rule.test(domain.replace(/^https?\:\/\/|\/$/g,''));
    ...
    isTrue(data = null) {
        ...
        if (typeof data == 'string')         result = (data === 'true')
        else if (typeof data == 'number')    result = (data === 1)
    ...
    isFalse(data = null) {
        ...
        if (typeof data == 'string')         result = (data === 'false')
        else if (typeof data == 'number')    result = (data === 0)
        else if (typeof data == 'boolean')   result = data
        else if (typeof data == 'object')    result = this.isEmpty(data)
    ...
    setCookie(name, value, exdays = 3600, domain = null, path = '/')
    {
        ...
        if (this.isEmpty(name)) result = '请设置 cookie 名称！'
            ...
            if (this.isEmpty(domain)) domain = window.location.hostname
    ...
    getCookie(name)
    {
        ...
        if (this.isEmpty(name)) result = '请输入需要查询的 cookie 名称！'
            ...
            if (begin !== -1) {
                ...
                // unescape
                result = decodeURIComponent(document.cookie.substring(begin, end));
            }
        }
        return result
    }
                    ...
                    image.onload   = function (e) {
                        ...
                        if ((new helper).isEmpty(config.maxWidth)) {
                            ...
                            if (width - height >= 0) {
                                ...
                                scale   = height / width;
                                let height1 = config.maxWidth;
                                let width1  = parseInt(height1 / scale);
                                if (height >= height1) {
                                    canvas.width  = width1;
                                    canvas.height = height1;
                                    ctx.drawImage(image, 0, 0, width1, height1);
     ...
     */
    customProcessApi(url = "", api = "api", cb = (item = {}) => {})
    {
        let result = url
     ...
     * @return {string|{red: number, rgba: string, green: number, blue: number}}
     */
    ...
    /**
     * @name  比较两个对象的指定字段是否相等
     * @param object1
     * @param object2
     * @param fields
     * @returns {boolean}
     */
    objectEqual(object1 = {}, object2 = {}, fields = [])
    {
     ...
     * @return {Object|boolean} tree [返回的树]
     */
    parseUrlQuery(url) {
        ...
        let [domaim, query] = [null, null]

        // 判断 url 是否包含 ?
        if (url.indexOf("?") !== -1) {
            // 获取 ? 之前的字符串
            domaim = url.substring(0, url.indexOf("?"))
            // 获取 ? 之后的字符串
            query = url.substring(url.indexOf("?") + 1)
        } else query = url

        const json = query.split("&").reduce((obj, pair) => {
        ...
        let result = { domian: {}, json }

        if (domaim) {
            // 通过正则分别匹配出协议、域名、端口、路径
            const [url, protocol, domain, port, path] = /^(https?:\/\/)?([^\/:]+)(:\d+)?(\/.*)?$/.exec(domaim)
            result.domian = {
                protocol: protocol.replace(/:\/\//, ''),
                value: domain, path, port: port ? port.replace(':', '') : '',
                yes: this.isDomain(domain)
            }
            if (this.isEmpty(result.domian.port)) {
                result.domian.port = result.domian.protocol === 'https' ? 443 : 80
            }
        }
    ...
    stringOmit(string = null, length = 10, omission = '...', location = 'right') {
        ...
        } else if (location === 'left' || location === 'start') {
            return omission + string.substring(string.length - length)
        } else {
            return string.substring(0, length / 2) + omission + string.substring(string.length - length / 2)
        }
    }
